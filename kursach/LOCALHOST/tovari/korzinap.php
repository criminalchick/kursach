...
$product_ids = array(); // Массив для хранения product_id
...
if (isset($_SESSION['user_id'])) { 
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') { 
        ...
        // Обработка формы при оформлении заказа 
            ...
            if (isset($_POST['selected_products'])) { 
                foreach ($_POST['selected_products'] as $product_id) { 
                    ...
                    if ($row = $result_select->fetch_assoc()) { 
                        ...
                        // Добавление product_id в массив 
                ...
                // Преобразование массива product_id в строку с разделителем запятая 
                $product_ids_string = implode(",", $product_ids); 

                // Вставка всех товаров в таблицу "zakazi" с одним запросом 
                $query_insert = "INSERT INTO zakazi (user_id, product_id, quantity) SELECT ?, product_id, quantity FROM korzina WHERE user_id = ? AND product_id IN ($product_ids_string)"; 
                $stmt_insert = $mysqli->prepare($query_insert); 
                $stmt_insert->bind_param("ii", $user_id, $user_id); 
                $stmt_insert->execute(); 
            } 
    ...
    while ($row = $result->fetch_assoc()) { 
        ...
        echo '<div class="image"> <img src="data:image/jpeg;base64,' . $row['image'] . '" alt="Product Image"> </div>'; 
